...
#define SPI_RESET_PIN  			GPIO3
#define GPIO_FIGERPRINT_RST  		GPIO3
...
static int __init spidev_init(void)
{
    ...
    if(status != 0)
        return status;
    printk("222222222222222222222222222222222222222222222222222222222,MKDEV(spidev_major, 0):%d\n",MKDEV(spidev_major, 0));
    spidev_class = class_create(THIS_MODULE, "spidev");
    if (IS_ERR(spidev_class)) {
        unregister_chrdev(spidev_major, spidev_spi_driver.driver.name);
        return PTR_ERR(spidev_class);
    }
    printk("33333333333333333333333333333333333333333333333333333333333333\n");
    status = spi_register_driver(&spidev_spi_driver);
    if (status < 0) {
        class_destroy(spidev_class);
        unregister_chrdev(spidev_major, spidev_spi_driver.driver.name);
    printk("444444444444444444444444444444444444444444444444444444444444444\n");
    }
    return status;
}
