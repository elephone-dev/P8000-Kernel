...
/*Note for tp esd check
* set A3_REG_VALUE for diff ic
*
*/
...
#define __MSG_DMA_MODE__
	...
	{
		g_dma_buff_va = (u8 *)dma_alloc_coherent(&tpd->dev->dev, 4096, &g_dma_buff_pa, GFP_KERNEL);//DMA size 4096 for customer
	    	if(!g_dma_buff_va)
		{
...
static int tpd_enable_ps(int enable)
{
	...
	TPD_PROXIMITY_DEBUG("[proxi_fts]read 22222: 0xB0's value is 0x%02X\n", state);
	return 0;
}
 ...
 static int tpd_get_ps_value(void)
  {
	  ...
	  int state = 0;
  	  ...
  	  msleep(20);
	  i2c_smbus_read_i2c_block_data(i2c_client, 0xB0, 1, &state);
	  TPD_PROXIMITY_DEBUG("proxi_5206 0xB0 state value is line = %d 0x%02X\n", __LINE__,state);
	  ...
	  TPD_PROXIMITY_DEBUG("proxi_5206 0x01 value is line = %d 0x%02X\n", __LINE__,proximity_status);
	  ...
	  TPD_PROXIMITY_DEBUG("tpd_proximity_detect line = %d = %d\n", __LINE__,tpd_proximity_detect);
...
int register_alsps(void)
{
                if((tpd_load_status == 0)||(fts_load_status == 0)) 
                        return -1;
		int err;
		...
		ps_ctl.enable_nodata = tpd_enable_ps;//ps_enable_nodata;
...
***********************************************************************/
 ...
 {	 
	...
	*/
	...
	/*#ifdef TPD_PROXIMITY
		{
			obj_ps.polling = 0;//1; //0--interrupt mode;1--polling mode;
			obj_ps.sensor_operate = tpd_ps_operate;
			printk("tpd_ps_operate 11111111111111\n");
