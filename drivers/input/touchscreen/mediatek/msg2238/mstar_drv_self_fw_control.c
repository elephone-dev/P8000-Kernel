...
#include "msg2233_s408_yijian_update_0307.h" // for MSG22xx
...
/*=============================================================*/
#ifdef TPD_PROXIMITY
extern int tpd_proximity_flag;
extern int g_nPsSensorDate;
#endif
u8 g_ChipType = 0;
...
#if defined(CONFIG_ENABLE_PROXIMITY_DETECTION)||defined(TPD_PROXIMITY) //TPD_PROXIMITY
#if defined(CONFIG_TOUCH_DRIVER_RUN_ON_SPRD_PLATFORM) || defined(CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM)
...
#elif defined(CONFIG_TOUCH_DRIVER_RUN_ON_MTK_PLATFORM)
             #ifdef TPD_PROXIMITY
			 if ( tpd_proximity_flag&& ((pPacket[5] == 0x80) || (pPacket[5] == 0x40)))
			 #elif defined(CONFIG_ENABLE_PROXIMITY_DETECTION)
                if (g_EnableTpProximity && ((pPacket[5] == 0x80) || (pPacket[5] == 0x40)))
			#endif
                {
                    int nErr;
                    ...
                    {printk("3333333333 line = %d\n",__LINE__);
			 			...
			 			printk("TPD_PROXIMITY  data = %d\n",g_nPsSensorDate);
						 ps_report_interrupt_data(g_nPsSensorDate);
